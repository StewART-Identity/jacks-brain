...
**Prepared by:** [[entities/jack-stewart]] | Architect/Engineer, [[entities/unt-system]] Identity and Access Management
**Date:** March 10, 2026
**Scope:** Part 0 (Pre-Production Testing) · Part 1 (Enterprise) · Part 2 (Departmental)

## Overview

A structured three-part rollout plan for two [[entities/microsoft-entra-id]] configuration changes at [[entities/unt-system]]:

1. Disabling [[concepts/system-preferred-mfa]] to prevent Entra ID from overriding [[entities/cisco-duo]] as the external authentication method (EAM)
2. Configuring [[concepts/mfa-sign-in-frequency]] to reduce daily MFA prompts (target: once per 7 days)
3. Restoring MFA enforcement for [[entities/citrix-horizon]] access via a new [[concepts/conditional-access-policy]]
...
- [[entities/cisco-duo]] configured as an [[concepts/external-authentication-method]] against a Duo sandbox
- [[concepts/conditional-access-policy|Conditional Access policies]] replicating `Logon-InternalUsers-CA`, `Logon-EntraAdminRoles-CA`, and the new `Logon-CitrixHorizon-CA`
- Enterprise app registration for [[entities/citrix-horizon]]
...
**Key test cases:** System-Preferred MFA override, [[concepts/mfa-sign-in-frequency|sign-in frequency + remembered devices]], risk-based policy with Duo EAM, `Logon-CitrixHorizon-CA` in report-only, EAM population expansion, auth strength for admins, app-specific session controls.
...
**Objective:** Reduce daily MFA prompts to once per 7 days for internal users; prevent Entra ID from overriding [[entities/cisco-duo]] with Microsoft Authenticator.

| Step | Change | Detail |
|------|--------|--------|
| 1 | Disable [[concepts/system-preferred-mfa]] | Set State to "Disabled." Target: DuoUsers group. |
| 2 | Configure Duo Remembered Devices | Enable in Duo Admin Panel for browser-based apps. Set to 7 days. |
| 3 | Set CA [[concepts/mfa-sign-in-frequency\|Sign-In Frequency]] | On `Logon-InternalUsers-CA`, enable sign-in frequency → 7 days. |
...
**Objective:** Restore MFA enforcement for [[entities/citrix-horizon]] for all Duo-enrolled users (~90,000).

| Step | Change | Detail |
|------|--------|--------|
| 1 | Sync DuoUsers Group | Added to Entra Connect sync scope (~90,000 members, refreshed every 12h). **DONE.** |
| 2 | Create `Logon-CitrixHorizon-CA` | New [[concepts/conditional-access-policy]]. Users: DuoUsers. Target: Citrix Horizon. Grant: Require MFA. Report-only first. |
...
Approved by [[entities/jack-stewart]] (Architect/Engineer, IAM), Parker Bush (Manager, IAM), and Ryan Kane (Director, Enterprise Collaboration Services).
