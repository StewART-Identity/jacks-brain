...
A persistent, licensed Microsoft [[entities/microsoft-entra-id]] infrastructure pattern using three parallel tenants: production, staging, and greenfield baseline. Proposed by [[entities/jack-stewart]] for [[entities/unt-system]] in February 2026.
...
- **Staging** needs P2 to test P2-dependent production features: risk-based [[concepts/conditional-access-policy|Conditional Access]], [[concepts/privileged-identity-management]], Identity Protection, access reviews. Testing against a P1 tenant produces a false sense of readiness.
...
This model provides the physical infrastructure for the [[concepts/iam-testing-methodology]]:
...
The January 28, 2026 ADFS → Entra ID migration, which affected 72,000+ users, forced the [[entities/unt-system]] IAM team to troubleshoot in production due to the absence of a test environment. The migration revealed undocumented behaviors around [[concepts/external-authentication-method]] registration cleanup and tenant-policy vs. user-object-level configuration discrepancies. See [[sources/2026-04-18-2026-04-18-entra-id-multi-tenant-environment-proposal]] for full background.
