...
A [[entities/microsoft-entra-id]] setting that, when set to "Microsoft managed," allows Entra ID to automatically select the MFA method it determines to be most secure or appropriate — overriding the user's default or a configured [[concepts/external-authentication-method]].
...
When System-Preferred MFA was set to "Microsoft managed," [[entities/microsoft-entra-id]] would present Microsoft Authenticator instead of [[entities/cisco-duo]] (the configured [[concepts/external-authentication-method]]) for some users. This undermined the organization's decision to use Duo as the primary MFA provider.
...
- Must be disabled before [[concepts/mfa-sign-in-frequency|sign-in frequency]] and [[concepts/conditional-access-policy|Conditional Access policy]] changes take effect as intended — otherwise Entra may swap the MFA method and invalidate the session controls.
