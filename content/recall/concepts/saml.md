...
3. The IdP presents a login page, validates credentials against a directory (e.g., [[recall/entities/active-directory|Active Directory]]), and issues an XML-based **SAML assertion**.
...
- XML-heavy; complex to implement compared to [[recall/concepts/oauth|OAuth]]/OIDC.
- Less suited for mobile and API-first architectures.
- Tightly coupled to browser-redirect flows.

## Relationship to OAuth

SAML handles **authentication** (who are you?) and conveys the result as an assertion. [[recall/concepts/oauth|OAuth]] handles **authorization** (what can you access?) and conveys the result as an access token. They solve adjacent but distinct problems; many enterprises run both. See [[recall/synthesis/saml-vs-oauth|SAML vs. OAuth — Comparison]].
