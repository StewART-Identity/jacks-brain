...
OAuth is primarily an **authorization** protocol (delegated access), whereas [[recall/concepts/saml|SAML]] is an **authentication + SSO** protocol. OpenID Connect (OIDC) extends OAuth 2.0 with an identity layer (ID Token) to cover the authentication gap, making OIDC the modern alternative to SAML for SSO. See [[recall/synthesis/saml-vs-oauth|SAML vs. OAuth — Comparison]].
