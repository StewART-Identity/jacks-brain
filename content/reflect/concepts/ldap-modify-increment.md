...
The LDAP Modify-Increment Extension ([[reflect/sources/2026-05-02-rfc4525-txt|RFC 4525]], June 2006) adds a fourth operation type — `increment (3)` — to the [[reflect/concepts/ldap|LDAP]] `ModifyRequest`, enabling clients to atomically add a delta to all existing values of an integer attribute. It eliminates the read-modify-retry loop previously required to increment counters in directory entries.
...
[[reflect/sources/2026-05-02-rfc4511-txt|RFC 4511]] defines three `ModifyRequest` operation type values:
...
Servers implementing Modify-Increment SHOULD publish `1.3.6.1.1.14` in the `supportedFeatures` attribute of the root DSE. Clients SHOULD check for this OID before using `increment`. The OID is in the IANA-assigned LDAP arc (`1.3.6.1.1`), consistent with the [[reflect/concepts/ldap-read-entry-controls|Read Entry Controls]] (`1.3.6.1.1.13.x`) and [[reflect/concepts/ldap-assertion-control|Assertion Control]] (`1.3.6.1.1.12`).
...
RFC 4525 is designed to compose with other [[reflect/concepts/ldap|LDAP]] controls:

**With [[reflect/concepts/ldap-read-entry-controls|Post-Read Control]] ([[reflect/sources/2026-05-02-rfc4527-txt|RFC 4527]])**: the increment and the read of the resulting value are atomic — the client learns the new assigned UID without a separate Search round trip.

**With [[reflect/concepts/ldap-assertion-control|Assertion Control]] ([[reflect/sources/2026-05-02-rfc4528-txt|RFC 4528]])**: the increment only executes if a precondition holds — yielding **test-and-increment** semantics. For example: "only increment if the counter is below a maximum bound."
...
Unlike the [[reflect/concepts/ldap-controls|LDAP controls mechanism]] (which augments the `LDAPMessage` envelope) or the `ExtendedRequest`/`ExtendedResponse` pattern (which introduces entirely new operations), Modify-Increment extends the **operation type enumeration** inside an existing operation. It is a rare LDAP extension pattern: modifying the core operation vocabulary of a base protocol operation rather than wrapping it. Discovery relies on `supportedFeatures`, the same mechanism used by [[reflect/sources/2026-05-02-rfc4526-txt|RFC 4526]] (Absolute True/False Filters).

For a cross-cutting analysis of how this fits among all five LDAP extension patterns, see [[reflect/synthesis/ldap-protocol-extensibility|LDAPv3 Extensibility: Controls, Features, and Companion RFCs]].
