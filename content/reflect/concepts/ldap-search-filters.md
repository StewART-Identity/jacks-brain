...
LDAP search filters are boolean predicate expressions that select entries from an [[reflect/concepts/ldap|LDAP]] directory. A filter is applied to a subtree of the directory tree and returns only entries whose attributes satisfy the predicate. On the wire, filters are ASN.1 BER-encoded per the LDAPv3 ASN.1 definition in RFC 4511; in human-readable contexts they use the string syntax specified in [[reflect/sources/2026-05-02-rfc4515-txt|RFC 4515]] (June 2006), which obsoletes the earlier [[reflect/sources/2026-05-02-rfc2254-txt|RFC 2254]] (December 1997).
...
[[reflect/sources/2026-05-02-rfc4526-txt|RFC 4526]] (June 2006) extends the filter model to restore absolute True and False assertions originally present in X.500 DAP but absent from LDAPv3. The rule follows directly from boolean algebra:
