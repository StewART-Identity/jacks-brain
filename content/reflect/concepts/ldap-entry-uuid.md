...
The UUID value is encoded as the standard dash-delimited ASCII string: `597ae2f6-16a6-1027-98f4-d28b5365dc14`. The associated `uuidMatch` [[reflect/concepts/ldap-matching-rules|matching rule]] supports equality filters on this form directly.
...
- [[reflect/concepts/distinguished-name|Distinguished Name]] — the mutable addressing mechanism that `entryUUID` complements
- [[reflect/concepts/ldap-matching-rules|LDAP Matching Rules]] — `uuidMatch` (equality) and `uuidOrderingMatch` (ordering) are the two matching rules RFC 4530 defines for UUID comparison
- [[reflect/concepts/ldap-syntaxes|LDAP Attribute Syntaxes]] — the UUID syntax (OID `1.3.6.1.1.16.1`) is a constrained form of Octet String, encoded as the RFC 4122 dash-delimited string in the LDAP protocol layer
- [[reflect/concepts/ldap-content-synchronization|LDAP Content Synchronization]] — SyncRepl depends on `entryUUID` to distinguish entry moves from delete/re-add pairs
