...
The **Directory Information Tree (DIT)** is the hierarchical tree structure used by X.500 and [[reflect/concepts/ldap|LDAP]] to organize all directory information. Every directory entry occupies a vertex in this tree; the relationships between entries are encoded in the tree's arcs. The DIT is the core organizing principle of the directory data model, introduced in the [[reflect/sources/2026-05-04-t-rec-x-500-198811-s-pdf-e|1988 X.500 recommendation]] and defined normatively in [[reflect/sources/2026-05-02-rfc4512-txt|RFC 4512]] based on X.501.
...
**Object entries** represent real-world objects — people, organizations, devices, or any identifiable entity. Each object entry belongs to exactly one structural [[reflect/concepts/ldap-object-classes|object class]] chain, which defines what the entry represents.
...
See [[reflect/concepts/distinguished-name|Distinguished Names]] for the string encoding rules. DNs unambiguously identify any entry in the DIT; no two entries may share the same DN.
...
Subschema definitions in the wiki's [[reflect/synthesis/ldap-schema-architecture|four-layer schema architecture]] — syntaxes, matching rules, attribute types, and [[reflect/concepts/ldap-object-classes|object classes]] — are all stored in and served from subschema subentries.

## Hierarchical Groups

A **hierarchical group** establishes an alternative hierarchical relationship among entries that is independent of the DIT tree structure. Defined in the [[reflect/sources/2026-05-04-t-rec-x-500-201610-s-pdf-e|2016 X.500 eighth edition]], hierarchical groups allow:
...
- Hierarchical relationships to change without altering the DIT structure, thereby preserving the [[reflect/concepts/distinguished-name|distinguished names]] of the entries involved
