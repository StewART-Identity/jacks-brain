...
**LDAP string preparation** is the normalization process applied to both stored attribute values and presented assertion values before [[reflect/concepts/ldap-matching-rules|matching rule]] evaluation. Defined in [[reflect/sources/2026-05-02-rfc4518-txt|RFC 4518]] and authored by [[reflect/entities/kurt-zeilenga|Kurt Zeilenga]], the algorithm ensures that character-by-character comparison of prepared strings produces the correct semantic result across implementations — regardless of encoding path, Unicode normalization form, or whitespace styling.
...
This is the most rule-specific step. It modifies the string to ensure proper handling of characters that the [[reflect/concepts/ldap-matching-rules|matching rule]] considers insignificant.
