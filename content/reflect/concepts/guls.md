...
The central construct in GULS is the `PROTECTED` [[reflect/concepts/asn1|ASN.1]] parameterized type, which wraps an application data type with one of several security transformation alternatives:
...
The [[reflect/sources/2026-05-04-t-rec-x-imp200-200612-i-msw-e|OSI Implementers' Guide v1.1]] documents three defects in X.830:
...
- In [[reflect/concepts/ldap|LDAP]], security is handled by **TLS** (transport-layer encryption via [[reflect/concepts/ldap-tls|LDAP TLS]]) and **SASL** (authentication via [[reflect/concepts/sasl|SASL]]) — completely separate from the protocol data encoding
