...
The **X.500 distributed directory model** describes how [[reflect/entities/itu-t|ITU-T]] X.500 deploys directory services across many systems and organizations to form "a single logical directory composed of many systems." This distributed architecture is a defining feature of the X.500 design and contrasts sharply with typical [[reflect/concepts/ldap|LDAP]] deployment practice, where chaining and multicasting are rarely implemented.
...
Both are OSI application protocols — application contexts composed of application service elements built on the Remote Operations Service (ROS) from X.219 — formally specified in X.519. Both use [[reflect/concepts/asn1|ASN.1]] encoding. Two additional protocols govern replication and operational bindings: DISP (Directory Information Shadowing Protocol, X.525) for [[reflect/concepts/x500-directory-replication|shadowing agreements]], and DOP (Directory Operational Binding Management Protocol) for administrative bindings between DSAs.

[[reflect/concepts/ldap|LDAP]] (Lightweight DAP) replaced DAP with a TCP/IP-native protocol starting with RFC 1487 (1993). No LDAP equivalent of DSP exists — LDAP clients receive referrals and chase them independently rather than relying on server-to-server chaining.
...
The gap between X.500's distributed vision and LDAP's simpler deployment model is one of the defining architectural differences between the two systems. The [[reflect/sources/2026-05-04-t-rec-x-500-201610-s-pdf-e|2016 eighth edition]] partially bridged this gap by formally defining LDAP requesters and responders as valid participants in chaining and referral topologies — but multicasting and DSP remain X.500-only. See [[reflect/synthesis/x500-ldap-convergence|X.500 and LDAP: Divergence and the 2016 Reconvergence]] for the full historical arc, and [[reflect/synthesis/x500-standards-maintenance-and-ldap-inheritance|X.500 Standards Maintenance]] for how the defect resolution process shaped shared infrastructure.
