...
The LDAP Password Modify Extended Operation, defined in [[reflect/sources/2026-05-02-rfc3062-txt|RFC 3062]] (February 2001), allows directory clients to update user passwords through a single LDAPv3 extended operation rather than through a standard Modify request. The operation is identified by OID `1.3.6.1.4.1.4203.1.11.1` (`passwdModifyOID`), registered in the [[reflect/entities/openldap-foundation|OpenLDAP Foundation]]'s IANA-assigned private enterprise arc.
...
This creates a layered dependency: RFC 3062 → [[reflect/concepts/ldap-tls|TLS/StartTLS]] → [[reflect/concepts/sasl|SASL]] authentication model — the same layers that [[reflect/sources/2026-05-02-rfc4513-txt|RFC 4513]] later formalized in the comprehensive LDAP security architecture.

## Extensibility Mechanism

The Password Modify operation uses the `ExtendedRequest` / `ExtendedResponse` mechanism — one of four distinct extensibility patterns in [[reflect/concepts/ldap|LDAPv3]]:

1. **Controls** — augment existing operations (e.g., [[reflect/concepts/ldap-paged-results|paged results]])
2. **ABNF grammar extension** — extend protocol argument syntax (e.g., [[reflect/concepts/ldap-search-filters|search filter]] syntax)
...
See [[reflect/synthesis/ldap-protocol-extensibility|LDAPv3 Extensibility: Controls, Features, and Companion RFCs]] for a cross-cutting analysis of these patterns across all cataloged RFCs.
