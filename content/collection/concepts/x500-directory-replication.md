...
The X.500 directory replication model, normatively defined in X.525 and overviewed in the [[collection/sources/2026-05-04-t-rec-x-500-201610-s-pdf-e|2016 eighth-edition X.500 overview]], describes how copies of [[collection/concepts/directory-information-tree|DIB]] information are maintained across multiple [[collection/concepts/x500-distributed-directory|DSAs]] to improve performance and availability. Three forms of replicated information are defined with distinct consistency guarantees.
...
- Relevant operational information including [[collection/concepts/x500-directory-access-control|ACI]] needed for full read access
...
Access control information (ACI) must always be replicated alongside the entries it governs. When entries are replicated to another DSA, the corresponding ACI travels with them, ensuring the shadow consumer can enforce the same [[collection/concepts/x500-directory-access-control|access control decisions]] as the master DSA.

## Relationship to LDAP Replication

[[collection/concepts/ldap|LDAP]] has no standard replication protocol in the core specification series. The X.500 DISP shadowing protocol is an OSI application protocol with no direct LDAP equivalent. [[collection/concepts/ldap-content-synchronization|LDAP Content Synchronization (RFC 4533, SyncRepl)]] is the closest LDAP analog — it provides change-notification-based replication between LDAP servers — but operates over the LDAP protocol rather than a separate shadowing protocol and is not interoperable with DISP.
