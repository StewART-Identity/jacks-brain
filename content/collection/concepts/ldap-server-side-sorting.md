...
LDAP server-side sorting is a [[collection/concepts/ldap-controls|LDAPv3 control]] extension defined in [[collection/sources/2026-05-02-rfc2891-txt|RFC 2891]] (August 2000). It allows a client to attach a `SortKeyList` to a `SearchRequest`, instructing the server to return results in a specified attribute order rather than in whatever order the server naturally produces.
...
- **`orderingRule`** (optional) — a [[collection/concepts/ldap-matching-rules|matching rule]] override; if absent, the attribute's defined ORDERING rule applies
...
When combined with [[collection/concepts/ldap-paged-results|paged results]] ([[collection/sources/2026-05-02-rfc2696-txt|RFC 2696]]), the server must sort the **entire** result set before returning any page. Sorting each page independently produces incorrect results. The sort key list must be identical across all search requests in the paged sequence, and the server SHOULD return a SortResult control on every `searchResultDone` in the sequence.
...
- [[collection/concepts/ldap-controls|LDAP Controls]] — the extension mechanism RFC 2891 uses
- [[collection/concepts/ldap-matching-rules|LDAP Matching Rules]] — the ordering rules used for sort key comparison
- [[collection/concepts/ldap-paged-results|LDAP Paged Results]] — the complementary pagination control; RFC 2891 §2.3 specifies their interaction
- [[collection/sources/2026-05-02-rfc2891-txt|RFC 2891]] — the normative specification
