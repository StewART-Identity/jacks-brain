...
The LDAP Assertion Control ([[collection/sources/2026-05-02-rfc4528-txt|RFC 4528]], June 2006) is an [[collection/concepts/ldap-controls|LDAP control]] that makes any [[collection/concepts/ldap|LDAP]] operation conditional on the evaluation of a [[collection/concepts/ldap-search-filters|Filter]] against the target entry. The `controlType` OID is `1.3.6.1.1.12`. The `controlValue` carries a BER-encoded `Filter` — the same ASN.1 type used in Search requests per RFC 4511 §4.5.1.
...
The control inherits the transport security of the underlying connection — see [[collection/concepts/ldap-tls|LDAP TLS]] for TLS layer protection. Beyond transport, three properties warrant attention:
...
Servers SHOULD publish OID `1.3.6.1.1.12` in the `supportedControl` attribute of the root DSE (per [[collection/sources/2026-05-02-rfc4512-txt|RFC 4512]]). A server MAY omit the advertisement and only grant access to authorized clients.

## Place in the Control Landscape

The Assertion Control is one of several controls in this wiki that augment [[collection/concepts/ldap|LDAP]] operations:

- [[collection/concepts/ldap-paged-results|Paged Results]] (RFC 2696): controls result-set chunking on Search
