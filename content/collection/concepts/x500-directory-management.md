...
**X.500 Directory Management** is the formal framework, defined in [[collection/sources/2026-05-04-t-rec-x-530-200811-i-pdf-e|ITU-T X.530 (2008) | ISO/IEC 9594-10]], for administering X.500 [[collection/concepts/x500-distributed-directory|Directory System Agents (DSAs)]] and Directory User Agents (DUAs) using [[collection/concepts/osi-systems-management|OSI Systems Management]] services and protocols.

The management framework is built on a key architectural division: X.500 directories have two kinds of manageable information. **Static** information (schema rules, prescriptive ACI) lives in the [[collection/concepts/directory-information-tree|DIB]] and is manageable via DAP. **Dynamic** information (counters, gauges, operational state) lives in a separate Management Information Base (MIB) and is manageable via [[collection/concepts/osi-systems-management|CMIP]].
...
[[collection/concepts/osi-systems-management|OSI Systems Management]] organizes managed objects in a containment tree. The X.530 hierarchy is:
...
The `dSA` managed object class is the central management point for [[collection/concepts/x500-distributed-directory|DSA]] administration. Its package exposes:
...
One `dseMO` instance corresponds to each DSA-specific entry in the DSA's naming context. It exposes the entry's knowledge information — specific knowledge (cross-references, subordinate references), non-specific knowledge (NSSRs), supplier/consumer knowledge for [[collection/concepts/x500-directory-replication|replication]], access point, DSE type, and timestamps.

### Shadowing Agreement Managed Object

Tracks [[collection/concepts/x500-directory-replication|replication agreements]] managed by a DSA:
...
[[collection/concepts/ldap|LDAP]] deployments have no equivalent standardized management layer. Each LDAP server vendor provides proprietary tools: CLI utilities, web consoles, vendor-specific extended operations, or SNMP MIBs with no interoperability. The absence of a standard like X.530 for LDAP means:
...
This management gap is one of the most significant structural differences between the full X.500 architecture and its LDAP successor. See [[collection/synthesis/x500-directory-management-vs-ldap-administration|X.500 Management vs. LDAP Administration]] for the full analysis, and [[collection/concepts/osi-systems-management|OSI Systems Management]] for the CMIP/CMIS framework underlying X.530's approach.
